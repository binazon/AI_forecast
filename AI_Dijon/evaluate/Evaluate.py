...
class Evaluate :
    ...
    def evaluateModel(self, dijon_train, label_train, dijon_test, label_test) -> str:
        ...
        model_eval ="Pertes(losses) sur le train : "+str(eval_train[0]) + "\n"
        ...
        model_eval+="Pertes(losses) sur le test : "+str(eval_test[0]) + "\n"
    ...
    '''
    def generating_errors(self, truth, predict):
        #forecast errors
        ...
        for i, y in enumerate(np.array(truth)):
            acc.append(abs(np.array(predict)[i] - y) / y if abs(y) > eps else 0)
